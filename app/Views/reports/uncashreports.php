...
<div id="container" style="padding:5px 5px 5px 5px;width:100%;">
	<div style="display:inline-block;float:left;width:40%;">
		...
		<!--
		<form id="reportform" method="post" action="<?=base_url('searchoc')?>">
	...
	<div id="tblCon" style="margin:5px 5px 5px 5px;">
		
		
		<table style="width:100%;font-size:20px;float:left;margin-top:10px;margin-bottom:10px;">
			<tr>
				<td colspan="9" align="center" style="width:100%;background-color:#4caf50;">Unschedules</td>
			</tr>
			<tr>
				...
				<td style="width:10%;background-color:#b3d7ff87;">Paid to</td>
				<td style="width:10%;background-color:#b3d7ff87;">Remark</td>
				<td style="width:10%;background-color:#b3d7ff87;">Action</td>
			</tr>
		<?php
		$i=1;
		foreach($uncashresult AS $uncash)
		{
				
		?>
			<tr>
				<td align="center" style="width:10%"><?=$i?></td>
				<td style="width:10%"><?=$uncash->ddate?></td>
				<td style="width:10%"><?=$uncash->invNo?></td>
				<td style="width:10%"><?=bankName($uncash->bname)?></td>
				<td style="width:10%"><?=$uncash->cno?></td>
				<td style="width:10%"><?=number_format($uncash->amount, 2)?></td>
				<td style="width:10%"><?=getdbc($uncash->pto)?></td>
				<td style="width:10%"><?=$uncash->remark?></td>
				<td style="width:10%">
					<a style="test-decoration: none;" title="Edit Record" href="<?=base_url('paymentschedupv/')?><?=$uncash->Id?>" target="_blank">Edit</a>
				</td>
			</tr>
		<?php
			$i++;
		...
		?>	
		<tr>
			<td align="center" colspan="8" style="">Total</td>
			<td style=""><?=number_format($untotal->totalAmount, 2)?></td>
		</tr>
		</table>
		<!--<div class="row" style="margin:5px 5px 5px 0px;width:100%; ;">	
		   <a style="" title="Update Current Record" href="<?=base_url('printpdfot/')?>" target="_blank"><input type="button"  name="btn_result" style="width:200px;" class="btn btn-primary line" value="Export PDF" /></a>
		</div>-->
	</div>
</div>	

<script>
//Bring search view sublayout for the OT
function searchFilter()
{ 
	var formData = $("#reportform").serialize(); 
	$.ajax({
				 type: 'post',
				 url: "<?=base_url('searchot')?>",
				 data: formData,
