<div id="sfilter" style="background-color: lavenderblush;clear:both;padding:5px 5px 5px 5px; margin-left:10px;margin-right:10px;border: 1px solid gray;display:block;margin-top:20px;">
    <form id="filter1" method="GET" action="">	
			...
			<div style="width:100%; margin:5px 5px 5px 5px;" class="row">
				...
				<div class="col-sm-6">
					<select  style="background-color:lavenderblush;width:100%;height:35px;text-align:center;font-weight:bold;"  id="emps" name="emps">
			...
			<div style="width:100%; margin:5px 5px 5px 5px;" class="row">
				<div class="col-sm-6">
					<select  style="background-color:lavenderblush;width:100%;height:35px;text-align:center;font-weight:bold;"  id="status" name="status" onchange="bringEmpByStatus(this.value);">
			...
			<div style="width:100%; margin:5px 5px 5px 5px;" class="row">
				<div class="col-sm-2" style="font-weight:bold;">
					<button type="button" onclick="filterSearch();" class="btn btn-info">Actual Punch Search</button>
				</div>
				<div class="col-sm-3" style="font-weight:bold;">
					<button type="button" onclick="filterSearch2();" class="btn btn-info">With Tolernace Punch Search</button>
				</div>
				<div class="col-sm-2" style="font-weight:bold;">
...
{  
	...
	$.ajax({
			 type: 'get',
			 url: "<?=base_url('attendance/report')?>",
			 data: formData,
...
//Filter search with tolerance
function filterSearch2()
{  
	var formData = $("#filter1").serialize(); 
	$.ajax({
			 type: 'get',
			 url: "<?=base_url('attendance/reportTol')?>",
			 data: formData,
			 success: function(result) {
				$("#attCon").empty(); 
				$("#attCon").append(result);
